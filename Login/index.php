...
if (isset($_POST['username']) && isset($_POST['pass'])) {
    ...
    $sql = "SELECT * FROM aspirants_mst WHERE login_email='$uname' AND password='$pass'";
    $result = mysqli_query($conn, $sql);

    if (mysqli_num_rows($result) == 1) {
            $row = mysqli_fetch_assoc($result);
            if($row['login_email'] == $uname && $row['password'] == $pass) {
                $_SESSION['userinfo'] = $row;
                header("Location: ../finowings-academy/");
                exit();
			}else{
                $_SESSION['error'] = "Incorrect Login Email or Password !!";
...
<html lang="en">
<head>
	<title>Login Page | Finowings Training Academy Pvt. Ltd.</title>
	<meta charset="UTF-8">
	   	...
	   	<div class="wrap-login100">
				<form class="login100-form validate-form p-l-55 p-r-55 p-t-120" method="post" action="">
					...
					<div align="center" class="wrap-input100 validate-input m-b-16" data-validate = "Please enter password">
						<span style="color: red; font-weight:500;">Incorrect Login Email or Password !!</span>	
					</div>
